...
class QinglongMonitorPlugin(BasePlugin):
    ...
    async def handle_ql_command(self, event: MessageEvent, args: str):
        """处理 .ql 命令"""
        if not args:
            await event.reply(await self.get_help())
            return
        ...
        if sub_cmd == "list":
            if cmd in ("cron", "crons"):
                ...
                await event.reply(f"未知资源类型：{cmd}，支持：cron, env, sub, dep")
        elif sub_cmd == "log":
            if cmd in ("cron", "crons"):
                await self._cmd_log(event, keyword)
            else:
                await event.reply("目前仅支持查看定时任务的日志：.ql cron log <任务名/ID>")
        elif sub_cmd == "run":
            if cmd in ("cron", "crons"):
                await self._cmd_run(event, keyword)
            else:
                await event.reply("目前仅支持运行定时任务：.ql cron run <任务名/ID>")
    ...
    async def _cmd_list_crons(self, event: MessageEvent, search: str):
        ...
        if not tasks:
            await event.reply("❌ 未获取到定时任务，请检查配置或青龙服务。")
            return
        status_map = {0: "✅ 空闲", 1: "🔁 运行中", 2: "❌ 失败", 3: "⏰ 超时"}
        lines = ["📋 定时任务列表："]
        for t in tasks:
    ...
    async def _cmd_log(self, event: MessageEvent, arg: str):
        if not arg:
            await event.reply("请提供任务名称或ID，例如：.ql cron log 签到")
            return
        tasks = await self.get_crons()
        if not tasks:
            await event.reply("无法获取任务列表")
            return
        target = None
        if arg.isdigit():
            for t in tasks:
        ...
        if not target:
            for t in tasks:
                if arg.lower() in t.get("name", "").lower():
                    ...
                    break
        if not target:
            await event.reply(f"未找到任务：{arg}")
            return
        log = await self.get_cron_log(target["id"])
        if log is None:
            await event.reply(f"获取日志失败，任务：{target['name']}")
            return
        if len(log) > 2000:
            log = log[-2000:] + "\n... (日志过长，仅显示末尾)"
        await event.reply(f"📄 任务【{target['name']}】最新日志：\n```\n{log}\n```")

    async def _cmd_run(self, event: MessageEvent, arg: str):
        if not arg:
            await event.reply("请提供任务名称或ID，例如：.ql cron run 签到")
            return
        tasks = await self.get_crons()
        if not tasks:
            await event.reply("无法获取任务列表")
        ...
        if ok:
            await event.reply(f"✅ 已触发运行任务：【{target['name']}】")
        else:
            await event.reply("❌ 触发运行失败")

    async def _cmd_monitor(self, event: MessageEvent, arg: str):
        if arg == "on":
            if event not in self.subscribers:
                self.subscribers.append(event)
                await event.reply("✅ 已开启定时任务状态推送（任务失败或完成时通知）")
            else:
                await event.reply("推送已开启")
        elif arg == "off":
            if event in self.subscribers:
                self.subscribers.remove(event)
                await event.reply("✅ 已关闭推送")
            else:
                await event.reply("未开启推送")
        else:
            await event.reply("用法：.ql monitor on  或  .ql monitor off")
